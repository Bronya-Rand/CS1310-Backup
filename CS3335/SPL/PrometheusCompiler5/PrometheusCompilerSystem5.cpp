...
========================
Added tokens
   (pseudo-terminals) FOR2 CHOOSE ONE ALL WHEN
   (  reserved words) 
   (     punctuation)
   (       operators)
...
||                          [ ELSE { <statement> }* ]
||                       END
||    *Note* <expression>s must be boolean
||       mode ONE: (default) means execute <statement>s for exactly ONE <expression> WHEN true
...
/*
||-----------------------------------------------------------
|| <FORStatement2> ::= FOR2 ( <assignmentStatement>  <expression>  <assignmentStatement> )
||                        { <statement> }*
||                     END
|| *Note* A PERIOD is "hidden" at end of each <assignmentStatement>
||    A way to make this <FORStatement> syntax "prettier" is to not require a PERIOD
||    at the end of each <assignmentStatement>, but require some other form of 
||    separator (perhaps COMMA) on either side of the <expression>. This would require
||    changing ParseAssignmentStatement() by adding the formal parameter
||    bool requirePERIOD = true and use additional logic to optionally recognize the
||    period, but only when requirePERIOD is true.
||-----------------------------------------------------------
*/
   void ParseAssignmentStatement(TOKEN tokens[]);
   void ParseExpression(TOKEN tokens[],DATATYPE &datatype);
