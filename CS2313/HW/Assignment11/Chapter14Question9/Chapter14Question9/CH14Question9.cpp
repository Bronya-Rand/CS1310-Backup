...
int main() {
	...
	if (!inOldMaster) {
		throw invalid_argument("Cannot open file 'oldmast.dat'.");
		exit(EXIT_FAILURE);
	}
	if (!inTransaction) {
		throw invalid_argument("Cannot open file 'trans.dat'.");
		exit(EXIT_FAILURE);
	}
	if (!outNewMaster) {
		throw invalid_argument("Cannot make file 'newmast.dat'.");
		exit(EXIT_FAILURE);
	}
